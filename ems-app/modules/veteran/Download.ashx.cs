...
namespace ems_app.modules.veteran
{
    ...
    public class Download : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var dt = GlobalUtil.GetDataTableWithParameters("SELECT *, RIGHT(FileName, CHARINDEX('.', REVERSE(FileName)) - 1) AS Extension FROM VeteranDocuments WHERE id = @ID", CommandType.Text, parameters);
            Byte[] bytes = null;
            if (dt.Rows.Count > 0)
            {
