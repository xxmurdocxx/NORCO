...
{
    public partial class CreditRecommendationsMultiple : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var selectedcreditremmendation = Session["SelectedCriteriaText"];
                Session["SelectedCreditRemmendation"] = selectedcreditremmendation;
                ...
                hfSelectedCriteria.Value = selectedcreditremmendation.ToString();
                hfAdvancedSarch.Value = selectedcreditremmendation.ToString();
                hfACECourseAdvancedSearch.Value = selectedcreditremmendation.ToString();
            ...
            {
                int count = this.rgCreditRecommendations.SelectedCells.Count;
                for (int i = 0; i < count; i++)
                {
                    var row = this.rgCreditRecommendations.SelectedCells[i].Text;
                    var column = this.rgCreditRecommendations.SelectedCells[i].Text;

                    //string text = Convert.ToString(this.rgCreditRecommendations.SelectedCells[i].Text.HeaderText);
                    //var cell = this.rgCreditRecommendations.SelectedCells[i];
                    //double val = Convert.ToDouble(cell.Text);
                }
            ...
            try
            {
                ...
                if (db.CheckIsDisabledForArticulate(Convert.ToInt32(hfSelectedCourse.Value)) == false)
                {
                    ...
                    if (rgCreditRecommendations.SelectedItems.Count > 0)
                    {
                        ...
                        {
                            rnMessage.Text = "Please select at least one Credit Recommendation";
                            rnMessage.Show();
                        }
        ...
        protected void rgCreditRecommendations_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            //foreach (GridDataItem item in rgCreditRecommendations.Items)
            //{
            //    string id = item["Criteria"].Text;
            //}

            //GridDataItem item = null;
            //if (e.Item is GridDataItem)
            //{
            //    string cri = Session["SelectedCriteriaText"].ToString();
            //    item = (GridDataItem)e.Item;

            //    string vaue4 = item.Cells[4].Text;
            //    if (vaue4 == cri)
            //    {
            //        if (item.ItemIndex == 0)
            //        {
            ...
            //foreach (GridDataItem items in rgCreditRecommendations.MasterTableView.Items)
            //{

            //     item = (GridDataItem)e.Item;
            //    string cri = Session["SelectedCriteriaText"].ToString();
            //    string txt = items.GetDataKeyValue("Criteria").ToString();
            //    if (cri == txt)
            //    {
            ...
            if (Session["SelectedAceID"] != null)
            {
                ...
                if (e.Item is GridDataItem && e.Item.OwnerTableView.Name == "ChildGridDetails")
                {
                    ...
                    if (item["ExhibitID"].Text == Session["SelectedAceID"].ToString() && (item["Criteria"].Text == Session["SelectedCriteriaText"].ToString()) || Session["SelectedCriteriaText"].ToString().Contains(item["Criteria"].Text))
                    {
                        item.Selected = true;
                        item.Enabled = true;
