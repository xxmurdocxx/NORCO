<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ImplementedArticulations.aspx.cs" Inherits="ems_app.modules.popups.ImplementedArticulations" %>
...
<head>
    ...
    <title>Implemented Articulations Report</title>
    <!-- Bootstrap -->
    ...
    <form id="form1" runat="server">
        ...
        <telerik:RadAjaxPanel ID="RadAjaxPanel1" runat="server" Width="100%" LoadingPanelID="RadAjaxLoadingPanel1" ClientEvents-OnRequestStart="onRequestStart">
           ...
           <asp:SqlDataSource ID="sqlType" runat="server" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" SelectCommand="select 'Course' as 'Type' union select 'Occupation' as 'Type'">
            ...
            <asp:SqlDataSource ID="sqlImplementedArticulations" runat="server" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" SelectCommand="select s.[subject], cif.course_number, cif.course_title, case when cif.Vunit_id is null then U.unit else concat(U.unit, ' - ', v.unit ) end as Units, d.department, cif.effective_start_date, cif.effective_end_date, case when a.ArticulationType = 1 then 'Course' else 'Occupation' end as 'Type', a.AceID, a.TeamRevd, acc.Title, [dbo].[GetHighlightRecommendationCriteria] (a.id) as 'Detail', [dbo].[GetArticulationMatrix](a.id) 'Matrix'  from  Articulation a join Course_IssuedForm cif on a.outline_id = cif.outline_id join LookupColleges c on cif.college_id = c.CollegeID join tblSubjects s on cif.subject_id = s.subject_id left outer join tblLookupUnits u on cif.unit_id = u.unit_id left outer join tblLookupUnits v on cif.Vunit_id = v.unit_id left outer join AceExhibit acc on a.ExhibitID = acc.ID left outer join tblLookupDepartments d on cif.department_id = d.department_id  where cif.college_id = @CollegeID and a.ArticulationStage = dbo.GetMaximumStageId(a.CollegeID) and a.ArticulationStatus = 1 and a.Articulate = 1 order by s.[subject], cif.course_number, a.AceID">
                <SelectParameters>
                    <asp:SessionParameter Name="CollegeID" SessionField="CollegeID" Type="Int32" />
                </SelectParameters>
            </asp:SqlDataSource>
            <div style="padding: 15px !important;">
                ...
                <telerik:RadGrid ID="rgImplementedArticulations" runat="server" AllowSorting="True" AutoGenerateColumns="False" Culture="es-ES" DataSourceID="sqlImplementedArticulations" AllowFilteringByColumn="True" AllowPaging="True" GroupingSettings-CaseSensitive="false" RenderMode="Lightweight" EnableHierarchyExpandAll="true"  OnPreRender="grid_PreRender">
                    ...
                    <MasterTableView Name="ParentGrid" DataSourceID="sqlImplementedArticulations" PageSize="12" CommandItemDisplay="Top" CommandItemSettings-ShowAddNewRecordButton="false" CommandItemSettings-ShowExportToExcelButton="true" AllowFilteringByColumn="true" AllowMultiColumnSorting="true" HeaderStyle-Font-Bold="true">
                        <CommandItemTemplate>
                        ...
                        <Columns>
                            <telerik:GridBoundColumn DataField="Matrix" Display="false" UniqueName="Matrix" AllowFiltering="false" AllowSorting="false" Exportable="false">
