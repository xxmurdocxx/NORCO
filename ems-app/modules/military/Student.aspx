...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <style>
        .credit-recommendation-needsfurtherreview {
    ...
    <asp:HiddenField ID="hfIDParent" runat="server" ClientIDMode="Static" />
    <asp:HiddenField ID="hfIDCreditRecomendationsAdjust" runat="server" ClientIDMode="Static" />
    <asp:HiddenField ID="hfEligibleCredits" runat="server" ClientIDMode="Static" />
    <telerik:RadWindowManager RenderMode="Lightweight" ID="RadWindowManager1" runat="server" DestroyOnClose="true" EnableViewState="false" EnableShadow="true" InitialBehaviors="Close" OnClientClose="closeRadWindow">
        <Windows>
            <telerik:RadWindow RenderMode="Lightweight" ID="modalUpload" runat="server" Width="720px" Height="240px" Behaviors="Close" Modal="true" OffsetElementID="main" Title="Upload Files" OnClientClose="OnClientCloseExclude">
                <ContentTemplate>
                    <div class="row">
                        <div class="col-3">
                        ...
                        <div class="col-3">
                            <asp:SqlDataSource ID="sqlDocumentType1" runat="server" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" SelectCommand="SELECT [documenttypeid],[documenttypeDescription] FROM [dbo].[DocumentTypes]" SelectCommandType="Text"></asp:SqlDataSource>
                            <telerik:RadDropDownList ID="ddlDocumenttypeUpload" DataSourceID="sqlDocumentType1" DataTextField="documenttypeDescription" DataValueField="documenttypeid" runat="server" 
                        ...
                        <div class="col-3">
                            ...
                            <telerik:RadAsyncUpload RenderMode="Lightweight" runat="server" ID="rauDocuments" AllowedFileExtensions="doc,docx,xls,xlsx,pdf,jpeg,jpg,png" TargetFolder="" MultipleFileSelection="Disabled"
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalNotes" runat="server" Title="ADD/EDIT NOTES" Width="550px" Height="300px" Behaviors="Close" Modal="true" VisibleStatusbar="false" OnClientClose="OnClientCloseExclude">
                <ContentTemplate>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalPopup" runat="server" Title="Search Articulations" Width="1250px" Height="720px" Modal="true" VisibleStatusbar="false" NavigateUrl="~/modules/popups/CreditRecommendations.aspx?SourceID=8" OnClientClose="OnClientCloseHandler">
            </telerik:RadWindow>
            <telerik:RadWindow RenderMode="Lightweight" ID="modalExclude" runat="server" Title="Exclude Articulated Courses" Width="1000px" Height="620px" Modal="true" VisibleStatusbar="false" NavigateUrl="~/modules/popups/ExcludeArticulationCourses.aspx" OnClientClose="OnClientCloseExclude" >
            </telerik:RadWindow>
            <telerik:RadWindow RenderMode="Lightweight" ID="modalPrint" runat="server" Title="Print CPL" Width="800px" Height="600px" Modal="true" VisibleStatusbar="false" OnClientClose="OnClientCloseExclude">
            </telerik:RadWindow>
            <telerik:RadWindow RenderMode="Lightweight" ID="modalExhibitACE" runat="server" Title="Add/Remove ACE Credit Recommendation" Width="1080px" Height="600px" Modal="true" VisibleStatusbar="false" OnClientClose="OnClientCloseExclude">
                <ContentTemplate>
                    <h3>Add ACE Credit Recommendation</h3>
                    <div class="row">
                        <div class="col-10">
                            <telerik:RadAutoCompleteBox ID="racbAceExhibitACE" runat="server" Width="100%" Filter="Contains" TextSettings-SelectionMode="Multiple" MinFilterLength="3" MaxResultCount="200" DropDownHeight="200" DataSourceID="sqlAdvancedSearchACE" DataTextField="FullCriteria" EmptyMessage="Search by Exhibit ID(s) or keyword(s)" DataValueField="CriteriaID" ClientIDMode="Static" AllowCustomEntry="false" HighlightFirstMatch="true" Delimiter="," AutoPostBack="true" ToolTip="" CssClass="acbCriteria" BackColor="#ffffe0" DropDownWidth="1000px"></telerik:RadAutoCompleteBox>
                            <asp:RequiredFieldValidator ID="rfvAceExhibitACE" runat="server" Display="Dynamic" ControlToValidate="racbAceExhibitACE" ValidationGroup="AddAceExhibit" ErrorMessage="* Required" ForeColor="Red" Font-Size="X-Small"></asp:RequiredFieldValidator>
                            <asp:SqlDataSource runat="server" ID="sqlAdvancedSearchACE" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" ProviderName="System.Data.SqlClient" 
                                SelectCommand="SELECT DISTINCT AE.ID, 
                                                      AE.AceID, 
                                                      AE.Title, 
                                                      AE.Exhibit, 
                                                      cast(AE.ID as nvarchar) + '|' + cast(EXC.CriteriaID as nvarchar) as 'CriteriaID', 
                                                      CONCAT(AE.AceID,  ' - ',CASE WHEN  AE.Exhibit IS NULL OR AE.Exhibit = '' THEN '' ELSE CONCAT('V : ' , AE.Exhibit)  END, CASE WHEN EXC.SkillLevel IS NULL OR EXC.SkillLevel = '' THEN '' ELSE CONCAT('SK : ' , EXC.SkillLevel)  END,  AE.Title, ' - ',	EXC.Criteria) FullCriteria 
                                                 FROM dbo.AceExhibit AE 
                                                INNER JOIN ACEExhibitCriteria AS EXC ON EXC.AceID = AE.AceID 
                                                                                    AND EXC.TeamRevd = AE.TeamRevd 
                                                                                    AND EXC.StartDate = AE.StartDate 
                                                                                    AND EXC.EndDate = AE.EndDate 
                                                                                    AND EXC.Criteria IS NOT NULL 
                                                WHERE EXC.SourceID = 1 --ACE
                                                ORDER BY AE.AceID" 
                                SelectCommandType="Text">
                            </asp:SqlDataSource>
                        </div>
                        <div class="col-2">
                            <asp:Button ID="btnAddAceExhibitACE" runat="server" Text="Add/Remove ACE Credit Recommendation" CssClass="btn" Width="180px" ValidationGroup="AddAceExhibit" OnClick="btnAddExhibitACE_Click" BackColor="#203864" ForeColor="White" />
                        </div>
                    </div>
                    <asp:SqlDataSource ID="sqlVeteranOccupationsACE" runat="server" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" CancelSelectOnNullParameter="false"  
                        SelectCommand="SELECT EX.ID AS Id, 
	                            EX.AceID + ' ' + EX.Title + ' V' + EX.VersionNumber + ' - ' + EXC.Criteria AS Title, 
	                            EXC.CriteriaID as CriteriaID FROM VeteranACECourse VC 
	                            JOIN ACEExhibit EX ON VC.ExhibitID = EX.ID
	                            JOIN ACEExhibitCriteria EXC ON VC.CriteriaID = EXC.CriteriaID
	                            WHERE VC.ManuallyAdded = 1 AND VC.VeteranId = @VeteranId">																												   
                        <SelectParameters>
                    ...
                    <telerik:RadGrid ID="rgVeteranOccupationsACE" runat="server" AllowSorting="True" AllowAutomaticDeletes="true" AutoGenerateColumns="False" Culture="es-ES" DataSourceID="sqlVeteranOccupationsACE" AllowFilteringByColumn="false" AllowPaging="false" GroupingSettings-CaseSensitive="false" AllowAutomaticInserts="true" ShowHeader="false">
                        <ClientSettings AllowKeyboardNavigation="true">
                            <Selecting AllowRowSelect="true"></Selecting>
                            <ClientEvents />
                        </ClientSettings>
                        <MasterTableView Name="ParentGrid" DataSourceID="sqlVeteranOccupationsACE" PageSize="12" DataKeyNames="Id" CommandItemDisplay="None" CommandItemSettings-ShowAddNewRecordButton="true" EnableHierarchyExpandAll="true" AllowFilteringByColumn="false" CommandItemSettings-AddNewRecordText="Add an Occupation" AllowMultiColumnSorting="true" EnableHeaderContextMenu="true">
                            <Columns>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalExhibit" runat="server" Title="Add/Remove MAP Exhibit" Width="1080px" Height="600px" Modal="true" VisibleStatusbar="false" OnClientClose="OnClientCloseExclude">
                <ContentTemplate>
                    <h3>Add/Remove MAP Exhibit</h3>
                    <div class="row">
                        <div class="col-10">
                            <telerik:RadAutoCompleteBox ID="racbAceExhibit" runat="server" Width="100%" Filter="Contains" TextSettings-SelectionMode="Multiple" MinFilterLength="3" MaxResultCount="200" DropDownHeight="200" DataSourceID="sqlAdvancedSearch" DataTextField="MAPAutoCompleteSearch" EmptyMessage="Search by Exhibit or keyword(s)" DataValueField="MAPAutoCompleteSearchID" ClientIDMode="Static" AllowCustomEntry="false" HighlightFirstMatch="true" Delimiter="," AutoPostBack="true" ToolTip="" CssClass="acbCriteria" BackColor="#ffffe0" DropDownWidth="1000px"></telerik:RadAutoCompleteBox>
                            <asp:RequiredFieldValidator ID="rfvAceExhibit" runat="server" Display="Dynamic" ControlToValidate="racbAceExhibit" ValidationGroup="AddExhibit" ErrorMessage="* Required" ForeColor="Red" Font-Size="X-Small"></asp:RequiredFieldValidator>
                            <asp:SqlDataSource runat="server" ID="sqlAdvancedSearch" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" ProviderName="System.Data.SqlClient" 
                                SelectCommand="SELECT DISTINCT AE.ID, 
                                                      AE.AceID, 
                                                      AE.Title, 
                                                      AE.Exhibit,
                                                      AE.ID MAPAutoCompleteSearchID, 
                                                      CONCAT(CONCAT(CONCAT(CONCAT(c.college,' / '), AE.AceID), ' '), AE.Title) MAPAutoCompleteSearch
                                                 FROM dbo.AceExhibit AE 
                                                INNER JOIN ACEExhibitCriteria AS EXC ON EXC.EXHIBITID = AE.ID AND EXC.Criteria IS NOT NULL
                                                INNER JOIN LookupColleges c ON c.collegeid = ae.collegeid
                                                WHERE EXC.SourceID = 4 AND AE.STATUS = 0
                                                ORDER BY AE.AceID" 
                                SelectCommandType="Text">
                            </asp:SqlDataSource>
                        </div>
                        <div class="col-2">
                            <asp:Button ID="btnAddAceExhibit" runat="server" Text="Add MAP Exhibit" CssClass="btn" Width="180px" ValidationGroup="AddExhibit" OnClick="btnAddExhibit_Click" BackColor="#203864" ForeColor="White" />
                        </div>
                    </div>
                    <asp:SqlDataSource ID="sqlVeteranOccupations" runat="server" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" CancelSelectOnNullParameter="false"  
                       SelectCommand="SELECT EX.ID AS Id, c.college + ' / ' + EX.AceID + ' ' + EX.Title + ' V' + EX.VersionNumber AS Title FROM ACEExhibit AS EX LEFT OUTER JOIN VeteranACECourse AS VC ON VC.AceID = EX.AceID INNER JOIN LookupColleges c ON c.collegeid = ex.collegeid WHERE VC.VeteranId = @VeteranId AND VC.CollegeId = @CollegeId AND EX.SOURCEID = 4 and EX.Status = 0"
                        DeleteCommand="DeleteVeteranMAPExhibit" OnDeleted="sqlVeteranOccupations_Deleted" DeleteCommandType="StoredProcedure">
                        <SelectParameters>
                            <asp:ControlParameter ControlID="hfCollegeID" PropertyName="Value" Name="CollegeId" Type="Int32" />
                            <asp:ControlParameter ControlID="hfVeteranID" PropertyName="Value" Name="VeteranId" Type="Int32" />
                        </SelectParameters>
                    ...
                    <telerik:RadGrid ID="rgVeteranOccupations" runat="server" AllowSorting="True" AllowAutomaticDeletes="true" AutoGenerateColumns="False" Culture="es-ES" DataSourceID="sqlVeteranOccupations" AllowFilteringByColumn="false" AllowPaging="false" GroupingSettings-CaseSensitive="false" AllowAutomaticInserts="true" ShowHeader="false">
                        ...
                        <MasterTableView Name="ParentGrid" DataSourceID="sqlVeteranOccupations" PageSize="12" DataKeyNames="Id" CommandItemDisplay="None" CommandItemSettings-ShowAddNewRecordButton="true" EnableHierarchyExpandAll="true" AllowFilteringByColumn="false" CommandItemSettings-AddNewRecordText="Add an Occupation" AllowMultiColumnSorting="true" EnableHeaderContextMenu="true">
                            <Columns>
                                ...
                                <telerik:GridTemplateColumn UniqueName="TemplateColumn" HeaderStyle-Width="30px" ReadOnly="true" AllowFiltering="false">
                                    <ItemTemplate>
                                        <div class="btn btn-primary" onclick="ShowConfirmDeleteMAPExhibits('<%#Eval("Id") %>', '' )"><i class="fa fa-trash" aria-hidden="true"></i> </div>
                                    </ItemTemplate>
                                </telerik:GridTemplateColumn>
                            </Columns>
                        </MasterTableView>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalNA" runat="server" Title="Not Applicable" Width="550px" Height="200px" Behaviors="Close" Modal="true" VisibleStatusbar="false" OnClientClose="OnClientCloseExclude">
                <ContentTemplate>
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col" style="text-align:right">
                            <telerik:RadButton ID="btnCancel" runat="server" Text="Cancel" OnClientClicked="CancelModalNA" AutoPostBack="false"></telerik:RadButton>
                        </div>
                    </div>
                </ContentTemplate>
            </telerik:RadWindow>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalApplicable" runat="server" Title="Make Applicable" Width="550px" Height="200px" Behaviors="Close" Modal="true" VisibleStatusbar="false">
                <ContentTemplate>
                    ...
                    <div class="row">
                        ...
                        <div class="col" style="text-align:right">
                            <telerik:RadButton ID="btnCancelAplicable" runat="server" Text="Cancel" OnClientClicked="CancelModalApplicable" AutoPostBack="false"></telerik:RadButton>
                        </div>
                    </div>
                </ContentTemplate>
            </telerik:RadWindow>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalConfirmReverseElective" runat="server" Title="Make Applicable" Width="550px" Height="220px" Behaviors="Close" Modal="true" VisibleStatusbar="false">
                <ContentTemplate>
                    ...
                    <div class="row">
                        ...
                        <div class="col" style="text-align:center">
                            <telerik:RadButton ID="btnCancelReverseElective" runat="server" Text="Cancel" OnClientClicked="CancelModalConfirmReverseElective" AutoPostBack="false"></telerik:RadButton>
                        </div>
                     </div>
                </ContentTemplate>
            </telerik:RadWindow>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalConfirmDeleteMAPExhibits" runat="server" Title="Delete confirmation" Width="550px" Height="220px" Behaviors="Close" Modal="true" VisibleStatusbar="false">
                <ContentTemplate>
                    <div class="row">
                        <div class="col" style="text-align: center">
                            <p>
                    ...
                    <div class="row">
                        <div class="col" style="text-align: right">
                            <telerik:RadButton ID="btnDeleteMAPExhibit" runat="server" Text="Yes" OnClick="btnDeleteMAPExhibit_Click" ></telerik:RadButton>
                        </div>
                        <div class="col" style="text-align: left">
                            <telerik:RadButton ID="RadButton3" runat="server" Text="Cancel" OnClientClicked="CancelModalConfirmMAPExhibit" AutoPostBack="false"></telerik:RadButton>
                        </div>
                    </div>
                </ContentTemplate>
            </telerik:RadWindow>            
            <telerik:RadWindow RenderMode="Lightweight" ID="modalAdjustCredit" runat="server" Title="Add / Edit Adjust Credit Recomendation" Width="1080px" Height="600px" Modal="true" VisibleStatusbar="false">
                <ContentTemplate>
                    <div class="row">
                        <div class="col-10">
                            <h3>Please select all credit recommendations you wish to combine to articulate with a single college course.</h3>
                        </div>
                        <div class="col-2" style="text-align: left">
                            <button onclick="buttonClick();" CssClass="btn btnprimary" style="width:160px">Combine Credit Recomendation</button>
                            <asp:Button ID="btnAdjustCredit" ClientIDMode="Static" runat="server" Text="XXX" CssClass="btn btnHiden" OnClick="btnAdjustCredit_Click" />
                        </div>
                    </div>
                    <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>"  
                        SelectCommand="GetVeteranCreditRecommendationsAdjusted" SelectCommandType="StoredProcedure" >
                        <SelectParameters>
                            <asp:ControlParameter ControlID="hfVeteranID" PropertyName="Value" Name="VeteranId" Type="Int32" />
                            <asp:ControlParameter ControlID="hfIDParent" PropertyName="Value" Name="IdParent" Type="Int32" />
                        </SelectParameters>
                    </asp:SqlDataSource>
                    <telerik:RadGrid ID="rgAdjustCreditsRecomendation" runat="server" AllowSorting="True" AllowAutomaticDeletes="true" AutoGenerateColumns="False" DataSourceID="SqlDataSource2" AllowPaging="false" AllowAutomaticInserts="true" ShowHeader="true">
                        <ClientSettings></ClientSettings>
                        <MasterTableView Name="ParentGrid" DataSourceID="SqlDataSource2" PageSize="12" DataKeyNames="Id">
                            <Columns>
                                <telerik:GridBoundColumn DataField="ID" UniqueName="Id" Display="false"  ReadOnly="true">
                                </telerik:GridBoundColumn>
                                <telerik:GridTemplateColumn DataField="IsSelected" UniqueName="IsSelected">
                                     <ItemTemplate>
                                        <asp:CheckBox runat="server" ID="chkBoolean" Enabled="true" Checked='<%# Eval("IsSelected") %>' />
                                    </ItemTemplate>
                                </telerik:GridTemplateColumn>
                                <telerik:GridBoundColumn DataField="CreditRecommendation" UniqueName="CreditRecommendation" HeaderText="Credit Recommendation" ReadOnly="true">
                                </telerik:GridBoundColumn>
                                <telerik:GridBoundColumn DataField="Criteria" UniqueName="Criteria" HeaderText="Criteria" ReadOnly="true">
                                </telerik:GridBoundColumn>
                            </Columns>
                        </MasterTableView>
                    </telerik:RadGrid>
            ...
            <telerik:RadWindow RenderMode="Lightweight" ID="modalNAGroup" runat="server" Title="Not Applicable (NA) Credit Recommendations" Width="1080px" Height="600px" Modal="true" VisibleStatusbar="false" OnClientClose="refreshGrid">
            </telerik:RadWindow>
        </Windows>
        ...
        <div class="row" style="white-space: nowrap">
            <div class="col text-center">
                ...
                <div id="divDocUpload" runat="server" title="CPL Document has been uploaded successfully (JST, Industry Certificate, AP Scores, etc.)">
                    <i aria-hidden="true" runat="server" id="DocUploadIcon" class="fa fa-check-circle" style="font-size: 2.5em;"></i>
            ...
            <div class="col text-center hideElement" title="AREA E (JST/DD-214 or waived) credits have been verified via course catalog and applied.">
            ...
            <div class="col text-center hideElement" title="Status of IMPLEMENTED articulations are accurate and true on student’s behalf.">
    ...
    <!-- STUDENT INFORMATION -->
   <telerik:RadPanelBar RenderMode="Lightweight" runat="server" ID="rpbStudentInfo" Width="100%" CssClass="mb-2">
            <Items>
                <telerik:RadPanelItem Text="Student Information" Expanded="true" EnableTheming="false" ToolTip="" CssClass="bg-light">
                   <HeaderTemplate>
                      <div class="d-flex justify-content-center align-items-center bg-primary">
                           <div class="col-12" style="color: #fff !important; font-size:20px; font-weight:400; text-align:center;margin-top:10px;">
                                 Student Information
                           
                              <a class="rpExpandable" style="color: #fff !important; display:block; width:30px; float:right;">
    ...
    <div class="row">
        <div class="col-12">
            <div class="row">
                ...
                <div class="col-1 col-1 d-flex justify-content-end align-items-center">
                    <telerik:RadLabel ID="rllProgram" runat="server" Font-Bold="true" Text="Program of Study:"></telerik:RadLabel>
                </div>
                <div class="col-2">
                    <asp:SqlDataSource ID="sqlPrograms" runat="server" ConnectionString="<%$ ConnectionStrings:NORCOConnectionString %>" SelectCommand="select program_id, program + ' - ' + cast(coalesce(tblLookupDegreeType.short_degree,'') as varchar(50))  + ' - ' + cast(coalesce(description,'') as varchar(50)) as program from Program_IssuedForm LEFT join tblLookupDegreeType ON Program_IssuedForm.degree_id = tblLookupDegreeType.degree_id where Program_IssuedForm.college_id=@CollegeId and Program_IssuedForm.status = 0 order by program">
                        <SelectParameters>
                            <asp:ControlParameter ControlID="hfCollegeID" PropertyName="Value" Name="CollegeId" Type="Int32" />
                        </SelectParameters>
                    </asp:SqlDataSource>
            ...
            <div class="row">
                ...
                <div class="col-1 d-flex justify-content-end align-items-center">
                    <telerik:RadLabel ID="RadLabel1" runat="server" Font-Bold="true" Text="Learning Mode:"></telerik:RadLabel>                    
                </div>
                <div class="col-2">
                    ...
                    <telerik:RadComboBox ID="rcbModelLearning" DataSourceID="sqlModelLearning" CssClass="mb-3" DataTextField="CPLModeofLearningDescription" DataValueField="id" runat="server" Width="100%" DefaultMessage="" DropDownAutoWidth="Enabled" EmptyMessage="Select a Model of Learning" CheckBoxes="true" BackColor="#ffffe0">
                        <Items>
    ...
    <asp:Panel ID="pnlStudentDocuments" runat="server">
        ...
        <div class="row">
                                ...
                                <fieldset class="fieldset itemFieldset">
                                    <div class="row">
                                        ...
                                        <div class="col-1 text-center">
                                            <asp:LinkButton ID="btnDownload" runat="server" ToolTip="Download/View file" OnClientClick=<%# Eval("id","javascript:window.open('/modules/popups/ConfirmDownload.aspx?ID={0}');return false;") %>> <i class="fa fa-download" aria-hidden="true" style="font-size: 1.7em; color:black"></i></asp:LinkButton>
                                        </div>
                                        <%--<div class="col-1 text-center">
            ...
            <div class="col-4">
                <div class="student-box">
                    <telerik:RadTextBox RenderMode="Lightweight" runat="server" ID="txtNotes" Width="100%" Height="200px" TextMode="MultiLine" Resize="None" EmptyMessage="Example: Student requested appeal the decision to set the credit recommendation for 3 Hours Communication, which was listed as Not Applicable. Student is seeking course credit."></telerik:RadTextBox><br />
                </div>
            </div>
            <div class="col-4">
                <div class="student-box">
    ...
    <div class="row" style="margin-bottom: 10px !important;">
        ...
        <div class="col-9 text-right" style="text-align: right;">
                ...
                <asp:Button ID="btnUpdateGrid" runat="server" Text="Save" CssClass="btnHiden" OnClick="btnUpdateGrid_Click" AutoPostBack="true" ClientIDMode="Static" />
        </div>
        <!-- SAVE STUDENT -->
    </div>
    <!-- SAVE STUDENT -->
    ...
    <asp:Panel ID="pnlMilitaryCredits" runat="server">
        ...
        <p class="p-2">Below are the credits for which the student is eligible. Some may already be articulated. For those that need an articulation click on action and choose the appropriate  option. </p>
                ...
                <CommandItemTemplate>
                    ...
                    <div class="commandItems">
                        <div style="float:left;"> 
                            ...
                            <telerik:RadButton runat="server" ID="btnExhibitACE" CommandName="ExhibitACE" Text="ADD/REMOVE ACE CREDIT RECOMMENDATION" ToolTip=""  Font-Bold="true">
                            </telerik:RadButton>
                            <telerik:RadButton runat="server" ID="btnExhibitCPL" CommandName="ExhibitCPL" Text="ADD/REMOVE MAP EXHIBIT" ToolTip=""  Font-Bold="true">
                            </telerik:RadButton>
                            <telerik:RadButton runat="server" ID="btnNewExhibit" CommandName="NewExhibit" Text="CREATE NEW MAP EXHIBIT" ToolTip=""  Font-Bold="true">
                            </telerik:RadButton>
                        </div>
                        ...
                        <ItemTemplate>
                            <telerik:RadDropDownList ID="ddlAction" runat="server" AppendDataBoundItems="true" Width="100%" AutoPostBack="True" OnSelectedIndexChanged="ddlAction_SelectedIndexChanged">
                                <Items>
                                    ...
                                    <telerik:DropDownListItem Text="Not Applicable (NA) Selected CR" Value="6" />
                                    ...
                                    <telerik:DropDownListItem Text="Exclude Articulated Courses" Value="11" />
                                    <telerik:DropDownListItem Text="Combine CRs" Value="12" />
                                    <telerik:DropDownListItem Text="Not Applicable (NA)" Value="13" />
  <%--                              <telerik:DropDownListItem Text="Adjust Credit Recomendation" Value="10" />--%>
                                </Items>
                            </telerik:RadDropDownList>
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
                    ...
                    <telerik:GridBoundColumn UniqueName="Criteria" DataField="Criteria" SortExpression="Criteria" HeaderText="CREDIT RECOMMENDATIONS (Units)" AllowFiltering="False" HeaderStyle-Font-Bold="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains" FilterControlWidth="150px"  Display="false">
                    </telerik:GridBoundColumn>
                    <telerik:GridBoundColumn UniqueName="CriteriaList" DataField="CriteriaList" SortExpression="CriteriaList" HeaderText="CREDIT RECOMMENDATIONS (Units)" AllowFiltering="False" HeaderStyle-Font-Bold="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains" FilterControlWidth="150px">
                    </telerik:GridBoundColumn>
                    ...
                    <telerik:GridBoundColumn SortExpression="AceExhibit" HeaderText="AceExhibit" DataField="AceExhibit"  UniqueName="AceExhibit" AllowFiltering="False" HeaderStyle-Width="200px" HeaderStyle-Font-Bold="true" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains" FilterControlWidth="150px" DefaultInsertValue="" ReadOnly="true" Display="false">
                    </telerik:GridBoundColumn>
                    ...
                    <telerik:GridBoundColumn UniqueName="Source" DataField="Source" SortExpression="Source" HeaderText="SOURCE" AllowFiltering="False" HeaderStyle-Font-Bold="true" HeaderStyle-Width="150px" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains" FilterControlWidth="150px">
                    </telerik:GridBoundColumn>
                    ...
                    <telerik:GridNumericColumn UniqueName="EligibleCredits" DataField="EligibleCredits" SortExpression="EligibleCredits" HeaderText="POTENTIAL CREDITS" AllowFiltering="False" HeaderStyle-Width="100px" HeaderStyle-Font-Bold="true" FilterControlWidth="40px" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" Aggregate="Sum" DecimalDigits="1" FooterAggregateFormatString="POTENTIAL CREDITS {0:###,##0.0}" FooterStyle-HorizontalAlign="Center" DataType="System.Double" DataFormatString="{0:###,##0.0}" EmptyDataText="" ReadOnly="true" HeaderTooltip="Available Value">
                    </telerik:GridNumericColumn>
                    <telerik:GridNumericColumn UniqueName="CreditsInReview" DataField="CreditsInReview" SortExpression="CreditsInReview" HeaderText="CREDITS IN REVIEW" AllowFiltering="False" HeaderStyle-Width="100px" HeaderStyle-Font-Bold="true" FilterControlWidth="40px" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" Aggregate="Sum" DecimalDigits="1" FooterAggregateFormatString="CREDITS IN REVIEW {0:###,##0.0}" FooterStyle-HorizontalAlign="Center" DataType="System.Double" DataFormatString="{0:###,##0.0}" EmptyDataText="" ReadOnly="true" HeaderTooltip="Available Value">
                    </telerik:GridNumericColumn>
                    ...
                    <telerik:GridNumericColumn UniqueName="Transcribed" DataField="Transcribed" SortExpression="Transcribed" HeaderText="TRANSCRIBED CREDITS" AllowFiltering="False" HeaderStyle-Width="100px" HeaderStyle-Font-Bold="true" FilterControlWidth="40px" HeaderStyle-HorizontalAlign="Center" ItemStyle-HorizontalAlign="Center" Aggregate="Sum" DecimalDigits="1" FooterAggregateFormatString="TRANSCRIBED CREDITS {0:###,##0.0}" FooterStyle-HorizontalAlign="Center" DataType="System.Double" DataFormatString="{0:###,##0.0}" EmptyDataText="" ReadOnly="true" HeaderTooltip="Unit Value will be shown once either the Articulation or Elective has been created">
                    </telerik:GridNumericColumn>
                    <telerik:GridTemplateColumn UniqueName="Notes" DataField="Notes" SortExpression="Notes" HeaderText="NOTES" AllowFiltering="False" HeaderStyle-Width="80px" HeaderStyle-Font-Bold="true" HeaderStyle-HorizontalAlign="Center" AutoPostBackOnFilter="true" CurrentFilterFunction="Contains" FilterControlWidth="150px">
                        <ItemTemplate>																					  				 
                            <div id="divNotes" runat="server" class="tooltip-element" title='<%# DataBinder.Eval(Container, "DataItem.Notes")%>' style="display: none">
...
<asp:Content ID="Content4" ContentPlaceHolderID="cphScripts" runat="server">
    ...
    <script type="text/javascript">
        ...
        function buttonClick() {
            var grid = $find("<%= rgAdjustCreditsRecomendation.ClientID %>");
            var masterTableView = grid.get_masterTableView();
            var ids = '';

            masterTableView.get_dataItems().forEach(function (row) {
                var chk = row.findElement("chkBoolean");
                if (chk.checked) {
                    var cell = row.get_cell('Id');
                    var cellValue = cell.innerText;
                    ids = ids + cellValue + ',';
                }
            });

            $('#hfIDCreditRecomendationsAdjust').val(ids);

            $('#btnAdjustCredit').click();
        }
        function OnClientCloseHandler(sender, args) {
            $('#btnUpdateGrid').trigger('click');
        }
        function OnClientCloseExclude(sender, args) {
            var radGrid = $find("<%= rgMilitaryCredits.ClientID %>");
        ...
        $(document).ready(function () {
            $('.tooltip-element').hover(function () {
                ...
                var tooltipWidth = $('.custom-tooltip').outerWidth();
                var tooltipHeight = $('.custom-tooltip').outerHeight();
                var leftPos = e.pageX - tooltipWidth - 10; // Adjust for tooltip width and desired offset
                var topPos = e.pageY - tooltipHeight - 10; // Adjust for tooltip height and desired offset
        ...
        function ShowConfirmDeleteMAPExhibits(id) {
            $('#hfDeleteId').val(id);
            var wnd = $find("<%= modalConfirmDeleteMAPExhibits.ClientID %>");
            wnd.show();
        }
        function CancelModalConfirmMAPExhibit() {
            var wnd = $find("<%= modalConfirmDeleteMAPExhibits.ClientID %>");
            wnd.hide();
            refreshGrid();
        }
