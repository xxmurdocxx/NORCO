...
{
    public partial class Confirm_o : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string Criteria = Decrypt(Request["Criteria"]);
                ...
                if (messages != null)
                {
                    if (messages.Rows.Count > 0)
                    {
                        foreach (DataRow row in messages.Rows)
                        {
                            ...
                            lblTitle.InnerText = row["Subject"].ToString() + " | ";
                            lblCriteria.InnerText = Criteria;
        ...
        protected void rgFacultyReviewArticulations_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridDataItem && grid.ID == "rgFacultyReviewArticulations")
            {
                ...
                string textTitle = dataBoundItem["Title"].Text;
                string textTitleRep = textTitle.Replace('\'', ' ');

                hp.NavigateUrl = $"javascript:showExhibit('{dataBoundItem["EntityType"].Text}','{dataBoundItem["AceID"].Text}','{dataBoundItem["TeamRevd"].Text}','{dataBoundItem["SelectedCriteria"].Text}','{dataBoundItem["StartDate"].Text}','{dataBoundItem["EndDate"].Text}',' {textTitleRep} ')";
                hp.Text = dataBoundItem["Title"].Text;
                hpAceID.NavigateUrl = $"javascript:showExhibit('{dataBoundItem["EntityType"].Text}','{dataBoundItem["AceID"].Text}','{dataBoundItem["TeamRevd"].Text}','{dataBoundItem["SelectedCriteria"].Text}','{dataBoundItem["StartDate"].Text}','{dataBoundItem["EndDate"].Text}','{textTitleRep}')";
        ...
        public void NotifyPopup(string action, string user_stage_id, string user_name, string user_id, string college_id, int width, int height)
        {
            ...
            var url = String.Format("/modules/popups/Notify.aspx?Action={0}&UserStageID={1}&UserName={2}&UserID={3}&CollegeID={4}&Notes={5}", action, user_stage_id, user_name, user_id, college_id, "");
            ...
            else
            {
                ...
                switch (Decrypt(Request["Action"]))
                {
                    ...
                    case "Deny":
                        ...
                        NotifyPopup("Denied", hvUserStage.Value, hvUserName.Value, hfUserID.Value, hvCollegeID.Value, 800, 600);
                        break;
                    default:
                        break;
                }
