...
{
    public partial class EmailAccess_o : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (authUser == 1)
                {
                    ...
                    if (Action == "ViewExhi")
                    {
                        ...
                        //string CriteriaPackageID = Decrypt(Request["CriteriaPackageID"]);
                        ...
                        if (Request["Criteria"] != null) {
                            Criteria2 = Decrypt(Request["Criteria"]);
                        }
                            

                        //string url = $"{host}/modules/Notifications/Confirm_o.aspx";
                        url = $"{host}/modules/Notifications/Confirm_o.aspx";
                        url_parameters = $"&MessageID={Encrypt(MessageID)}&AceID={Encrypt(user)}&TeamRevd={Encrypt(TeamRevd)}&ActionTaken={Encrypt(ActionTaken)}&Criteria={Encrypt(Criteria2)}";
                ...
                else
                {
                    if (authUser == -1)
                    {
                        ...
                        FailureText.Text = "<h3 class='d-flex justify-content-start' style='font-weight:normal;color:darkblue !important;'>This link has expired, please log into MAP below</h3>";
                    }
                    else
                    {
                        //Login1.FailureText = "<a href='mailto:mapadmin@militaryarticulationplatform.org' style='font-weight:normal;color:darkblue !important;'>Your account has been deactivated. If you feel this is an error, please contact MAP Support.</a>";
                    }
                }
        ...
        //    string url = "";
        //    string host = GlobalUtil.GetDatabaseSetting("SiteName");
        //    url = $"{host}/modules/security/Login.aspx";
        ...
        protected void Login_Click(object sender, EventArgs e)
        {
            string href = "";
            string host = GlobalUtil.GetDatabaseSetting("SiteName");
            href = $"{host}/modules/security/Login.aspx";
            Response.Redirect(href);
        }
        ...
        protected void btnSend_Click(object sender, EventArgs e)
        {
            ...
            //        lblRememberPassword.Text = string.Format("<p>Your password has beeen successfully sent to {0}</p>", rtbUsername.Text.ToString());
